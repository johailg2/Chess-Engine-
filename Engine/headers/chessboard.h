...
// file masks for preventing wraparound
...
// square indices - LERF mapping (bit 0 = h1, bit 63 = a8)
enum ChessSquare {
...
struct Move {
    ...
    Move(int ss, int es, PieceType mpt, Color mpc, Piece cp, 
        ...
        : startingSquare(ss), endSquare(es), movingPieceType(mpt), movingPieceColor(mpc), 
          capturedPiece(cp), promotedPieceType(ppt), isPromotion(prom), 
...
public:
    ...
    // these are public for eval to use
    ...
    // public members for eval access
...
private:
    // boards array: 0=pawn, 1=rook, 2=knight, 3=bishop, 4=queen, 5=king
    ...
    // FIXED: typos corrected but keeping old names as aliases for compatibility
    void initializeAllSquaresAttackedByWhite();
    void initializeAllSquaresAttackedByBlack();

    vector<Move> enPassantEnable;
    
    void initializeFileMasks();
    void initializeRankMasks();
